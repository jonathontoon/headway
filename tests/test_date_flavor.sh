...
# is_valid_date, plus detect_date_flavor's 3-way fallthrough when the host
# date supports the GNU/BusyBox `-u -d` shape.
#
# The BusyBox branch only uses the GNU/BusyBox-compatible `date -u -d "@N"` /
# `date -u -d "YYYY-MM-DD"` subset. macOS/BSD date does not implement that
# shape, so those branch-specific checks are skipped there.
...
if date -u -d "@0" "+%Y-%m-%d" >/dev/null 2>&1; then
	...
	# A fake date shim rejects GNU's "-d 1 day" and BSD's "-v+1d" probes but
	# delegates everything else to the real date.
