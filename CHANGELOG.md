...
- `clear <due|priority|tags|project> <id> [<id>...]` empties a field on one or more tasks — no more reaching for `$EDITOR` to remove a value. `clear tags <id> @tag [@tag...]` removes specific tags from a single task; the bare form wipes every tag. `tag <id> @tag [@tag...]` now accepts multiple tags in one call. No magic `none` sentinel and no `-@tag` shorthand — a dedicated verb is easier to discover, generalises across all four fields, and can't collide with real content.
...
- Source split into `src/00-preamble.sh` through `src/09-main.sh`; `headway.sh` is now a build artefact generated by `make build` (which concatenates `src/*.sh` in filename order). The bundle is still committed so `curl | sh` and Homebrew keep working. A `make verify` target catches drift by diffing `headway.sh` against a fresh build.
- Extracted a `report_change <verb> <id> <line>` helper for the confirmation output that every mutating command emits. Removed dead `shell_summary`.
